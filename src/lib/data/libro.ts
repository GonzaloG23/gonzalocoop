...
export async function cargarParametros(): Promise<ParametrosControl> { if (!supabaseConfigured()) { try { const guardados = JSON.parse(localStorage.getItem(DEMO_PARAMETROS_KEY) ?? "null") as ParametrosControl | null; if (guardados && Number.isFinite(Number(guardados.dia_limite_cierre)) && Number.isFinite(Number(guardados.tope_egreso))) return guardados; } catch {} return PARAMETROS_POR_DEFECTO; } const { data, error } = await supabase.from("parametros_control").select("id, dia_limite_cierre, tope_egreso").order("created_at").limit(1).maybeSingle(); if (error) throw error; return (data as ParametrosControl | null) ?? PARAMETROS_POR_DEFECTO; }
export async function guardarParametros(p: { id: string; dia_limite_cierre: number; tope_egreso: number }) { if (!supabaseConfigured()) { const parametros: ParametrosControl = { id: p.id || "demo-parametros", dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso }; localStorage.setItem(DEMO_PARAMETROS_KEY, JSON.stringify(parametros)); return; } if (p.id) { const { error } = await supabase.from("parametros_control").update({ dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso }).eq("id", p.id); if (error) throw error; return; } const { error } = await supabase.from("parametros_control").insert({ dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso }); if (error) throw error; }
