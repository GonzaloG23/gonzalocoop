...
export async function guardarParametros(p: { id: string; dia_limite_cierre: number; tope_egreso: number; saldo_minimo_cuenta_bancaria: number }) {
  if (usingMinisterioApi()) return ministerioRequest<ParametrosControl>("/api/parametros-control", { method: "PATCH", body: JSON.stringify({ id: p.id, dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso, saldo_minimo_cuenta_bancaria: p.saldo_minimo_cuenta_bancaria }) });
  if (!supabaseConfigured()) { const parametros: ParametrosControl = { id: p.id || "demo-parametros", dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso, saldo_minimo_cuenta_bancaria: p.saldo_minimo_cuenta_bancaria }; localStorage.setItem(DEMO_PARAMETROS_KEY, JSON.stringify(parametros)); return; } if (p.id) { const { error } = await supabase.from("parametros_control").update({ dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso, saldo_minimo_cuenta_bancaria: p.saldo_minimo_cuenta_bancaria }).eq("id", p.id); if (error) throw error; return; } const { error } = await supabase.from("parametros_control").insert({ dia_limite_cierre: p.dia_limite_cierre, tope_egreso: p.tope_egreso, saldo_minimo_cuenta_bancaria: p.saldo_minimo_cuenta_bancaria }); if (error) throw error; }