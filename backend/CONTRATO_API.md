...
El frontend actualmente utiliza el endpoint institucional con la forma:
`POST /api/cooperadoras/:id/periodos/:periodoId/cerrar`.

La implementación de transición del adaptador puede utilizar una ruta simplificada mientras se valida el backend, pero antes de activar `VITE_DATA_BACKEND=ministerio-api` deberá quedar unificada con este contrato.
